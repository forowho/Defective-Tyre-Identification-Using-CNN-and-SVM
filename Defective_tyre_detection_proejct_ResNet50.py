...
# ### Convolutional neural network model with 50 layers deep (ResNet50)
# This notebook looks at defective tyre detection. The dataset used has 2 classes: `Defective Tyre`, and `Good Tyre`
# 
# The model implemented uses a ResNet50 layer for feature extraction on all the images. The features are then fed to machine learninng models like SVM & random forest for binary classification.
...
train_features = train_features.reshape(train_size, 5*5*2048)
validation_features = validation_features.reshape(val_size, 5*5*2048)

# save to csv file
np.savetxt('X_train_defective.csv', train_features, delimiter=',')
np.savetxt('X_val_defective.csv', validation_features, delimiter=',')
np.savetxt('y_train_defective.csv', train_labels, delimiter=',')
np.savetxt('y_val_defective.csv', validation_labels, delimiter=',')
...
train_features = np.loadtxt('X_train_defective.csv', delimiter=",")
train_labels = np.loadtxt('y_train_defective.csv', delimiter=",")
validation_features = np.loadtxt('X_val_defective.csv', delimiter=",")
validation_labels = np.loadtxt('y_val_defective.csv', delimiter=",")
...
parameters = {
    'C': [0.0001, 0.001, 0.01, 0.1, 1],  # Regularization parameter
...
parameters = [{'min_samples_split': np.arange(10,40,10),
